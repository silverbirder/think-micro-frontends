  ...
  render() {
    ...
    return (
      <div>
        ...
        <p>
          Color value in <strong>ColorApp</strong>:{" "}
        ...
        <p>
          Counter value from <strong>CounterApp</strong>:{" "}
          <code>{this.props.counter}</code>
        </p>

        <p>
          <a href="#" onClick={() => this.props.incrementCounter()}>
            Increment
          </a>{" "}
          counter from here.
        </p>
        ...
        <div>
          ...
          <pre>
            <code>{JSON.stringify(this.props.regionProps, null, 2)}</code>
          </pre>
        </div>
        ...
        <div>
          ...
          <ul>
            <li>
              <strong>Foo</strong> (cascaded): is from{" "}
              <code>{this.props.foo.getAppName()}</code>
            </li>
            <li>
              <strong>Bar</strong> (cascaded and scoped): is from{" "}
              <code>{this.props.bar.getAppName()}</code>
            </li>
            <li>
              <strong>Baz</strong> (not cascaded): is unavaialble -{" "}
              <code>{this.props.baz}</code>
            </li>
          </ul>
        </div>
      </div>
...
export default observe(function (app) {
  ...
  const state$ = store.getState$()::map((state) => {
    return {
      color: state.color.value,
    };
  });

  const regionProps$ = region.getProps$()::map((regionProps) => {
    return {
      regionProps,
    };
  });
  ...
  const counterAppActions$ = counterApp$::map((counterApp) => {
